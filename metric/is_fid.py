...
class ScoreModel:
    ...
    def get_score_image_tensor(self, imgs_nchw, mu1=0, sigma1=0,
                               n_split=1, batch_size=1, return_stats=False,
    ...
    def get_score_dataset(self, dataset, mu1=0, sigma1=0,
                          n_split=1, batch_size=32, return_stats=False,
                          return_each_score=False):
        """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpus', default=None, type=int, help='gpu_number')
    parser.add_argument('--classifier',  action='store_true') #default == Fasle
    args = parser.parse_args()
    if (args.model != 'glide'):
        parser.add_argument('--path', type=str, default=f'./data_gen/{args.model}/image', help='Path to the generated images or to .npz statistic files')
        parser.add_argument('--save_results', type=str, default=f'./results/{args.model}.txt', help='Path to the save results of IS, FID scores')
    elif (args.classifier):
        parser.add_argument('--path', type=str, default=f'./data_gen/{args.model}/image/cond', help='Path to the generated images or to .npz statistic files')
        parser.add_argument('--save_results', type=str, default=f'./results/{args.model}_cond.txt', help='Path to the save results of IS, FID scores')
    else:
        parser.add_argument('--path', type=str, default=f'./data_gen/{args.model}/image/free', help='Path to the generated images or to .npz statistic files')
        parser.add_argument('--save_results', type=str, default=f'./results/{args.model}_free.txt', help='Path to the save results of IS, FID scores')
    #parser.add_argument('--fid', type=str, default='', help='Path to the generated images or to .npz statistic files')
    parser.add_argument('--fid', type=str, default='./data/image', help='Path to the generated images or to .npz statistic files')
    ...
    # if argv have foldername/, calc IS score of pictures in this folder
        ...
        if args.fid.endswith('.npz'):
            ...
            print('IS score =', is_mean)
            print('FID =', fid)
            
            # Save result to file
            with open(args.save_results, "a") as f:
                f.write(f"IS: {is_mean}")
                f.write('\n')
                f.write(f"FID: {fid}")
        ...
        # args.fid == a foldername/
            ...
            print('1st IS score =', is_mean1)
            print('2nd IS score =', is_mean2)
            print('FID =', fid)
        
            # Save result to file
            with open(args.save_results, "a") as f:
                f.write(f"1st IS score: {is_mean1}")
                f.write('\n')
                f.write(f"2nd IS score: {is_mean2}")
                f.write('\n')
                f.write(f"FID: {fid}")
                f.write('\n')
